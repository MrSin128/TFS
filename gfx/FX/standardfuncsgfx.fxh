...
PixelShader = 
{
	...
	float3 GetMudColor( in float3 vResult, in float4 vMudSnowColor, in float3 vPos, inout float3 vNormal, inout float vGlossiness, inout float vSpec, in sampler2D MudDiffuseGlossSampler, in sampler2D MudNormalSpecSampler )
	{
	...
	*/
	...
	float CalculateOccupationMask2( in float2 uv )
	{
		// diagonal
		float t = 3.14159 / 8.0;	    
		float w = SEC_MAP_TILE * 0.5;			  // larger value gives smaller width
	...
	void secondary_color_mask( inout float3 vColor, float3 vNormal, float2 vUV, in sampler2D TexMask, inout float vBloomAlpha )
	{
		...
		if (vColorMask.a > 0 && vColorMask.r * 0.375 < vColorMask.g && vColorMask.r * 0.44 > vColorMask.g ) {
			float vOccupationMask = CalculateOccupationMask2( vUV );
			if (vOccupationMask> 0.5) {
				float factor = (0.45f * vColorMask.a);
				vColor = lerp( vColor, float3(0, 0, 0), factor );
			} else {
				float factor = (0.65f * vColorMask.a);
				vColor = lerp( vColor, float3(0, 0, 0), factor );
			}

			if (vColorMask.a > 0.2 && vColorMask.a < 0.4) {
				vColor = lerp(float3(0, 0.17, 0.77), vColor, abs(vColorMask.a - 0.3) / 0.1);
			}
